...
const FORECAST_EXPORT_CAMPAIGNS = [
 ...
 'Leads | VISA | CA | KHM',
 'Leads | VISA | KR | KHM',
 'Leads | VISA | CN+HK | KHM',
 'Leads | VISA | ALL | FOR',
 'Leads | VISA | UK | KHM',
 'Leads | VISA | EU | KHM',
 'Leads | VISA | ALL | KHM',
 'Engagement | VISA | ALL | KHM',
 'Leads | VISA | JP | KHM',
 'Leads | VISA | JP | FOR',
 'Leads | VISA | AU | FOR',
 'Leads | VISA | HK | FOR',
 ...
 'Leads | VISA | US | KHM',
 'Leads | VISA | EU | FOR',
 'Engagement | VISA | ALL | FOR',
] as const;

const campaignNameKey = (value: unknown) => String(value || '')
 .trim()
 .replace(/\s*\|\s*/g, '|')
 .toLocaleLowerCase();
...
function ForecastCsvExport({
 ...
 const campaignOptions = useMemo<ForecastCampaignExportOption[]>(() => {
  const sourceIdsByName = new Map<string, Set<string>>();
  const addSourceId = (nameValue: unknown, idValue: unknown) => {
   const key = campaignNameKey(nameValue);
   const id = String(idValue || '').trim();
   if (!key || !id) return;
   const ids = sourceIdsByName.get(key) || new Set<string>();
   ids.add(id);
   sourceIdsByName.set(key, ids);
  };
  campaigns.forEach((campaign: any) => {
   const sourceIds = (Array.isArray(campaign.scopeCampaignIds)
    ? campaign.scopeCampaignIds
    : String(campaign.campaign_id || '').split(','))
    .map((value: unknown) => String(value).trim())
    .filter(Boolean);
   sourceIds.forEach((id: string) => addSourceId(campaign.campaign, id));
  });
  (adSpend?.daily_campaigns || []).forEach((row: any) => addSourceId(row.campaign_name, row.campaign_id));
  return FORECAST_EXPORT_CAMPAIGNS.map((name) => {
 ...
 const selectedCampaignSourceIds = useMemo(() => new Set(
  campaignOptions
 ...
 const exportRows = useMemo(() => {
  ...
  return source.filter((row: any) => {
   ...
   const selected = scope === 'campaign' ? selectedCampaignSourceIds.has(id) : selectedSet.has(id);
   return selected && (!startDate || day >= startDate) && (!endDate || day <= endDate);
  }).sort((a: any, b: any) => (
   String(a.day).localeCompare(String(b.day))
   || String(a.campaign_name || '').localeCompare(String(b.campaign_name || ''))
   || String(a.ad_set_id || '').localeCompare(String(b.ad_set_id || ''))
  ));
 }, [adSpend, scope, selectedCampaignSourceIds, selectedSet, startDate, endDate]);
 ...
 const openDialog = () => {
  ...
  const currentCampaignIds = new Set(String(currentCampaignId || '').split(',').map((id) => id.trim()).filter(Boolean));
  const candidateIds = nextScope === 'adset'
   ? [String(currentAdSetId)]
   : campaignOptions
    .filter((option) => option.sourceIds.some((id) => currentCampaignIds.has(id)))
...
function ForecastPage({ role }: { role: UserRole }) {
 ...
 {spendFittedCurveBand.length > 1 && <span><i className="band" />95% confidence band</span>}
...
function FollowupPage() {
 ...
 }, [offset, search, stage, due, assigned, platform, campaign, sort, direction]);
 ...
 const clearFilters = () => {
  setSearchDraft(''); setSearch(''); setStage(''); setDue(''); setAssigned(''); setPlatform(''); setCampaign('');
 };
 const hasFilters = !!(search || stage || due || assigned || platform || campaign);
 const activeFilterCount = [stage, due, assigned, platform, campaign].filter(Boolean).length;
  ...
  <div className="page-content followup-page">
   ...
   <section className="followup-board">
    <div className="followup-toolbar followup-compact-filters">
     ...
     />
     <MenuSelect
      ariaLabel="Filter by campaign name"
      value={campaign}
      className={`followup-pill-select followup-campaign-select${campaign ? ' is-scoped' : ''}`}
      options={[{ value: '', label: 'All campaigns', icon: Layers3 }, ...(rowsData.facets.campaigns || []).map((item: any) => ({ value: item.value, label: item.label || item.value, icon: Layers3 }))]}
      onChange={setCampaign}
     />
     <details className={`followup-filter-menu followup-person-filter${assigned ? ' is-scoped' : ''}`}>
      <summary><UserCheck size={15} /><span>{assigned || 'Anyone'}</span>{activeFilterCount > 0 && <b>{activeFilterCount}</b>}<ChevronDown size={13} /></summary>
     ...
     <MenuSelect ariaLabel="Sort follow-ups" value={`${sort}:${direction}`} className="followup-sort-select followup-pill-select" options={[
      ...
      { value: 'campaign:asc', label: 'Campaign name', short: 'Sort', icon: ArrowUpDown },
...
function LeadManagementPage({ role }: { role: UserRole }) {
 ...
 const spendOutcomeScopeLabel = adSetId
  ? `Ad set ${adSetId}`
  : campaignId
   ? (selectedCampaignChoice?.campaign || 'Selected campaign')
 ...
 const spendOutcomeChart = [
  ...
  },
  {
   ...
   share: Number(stages.find((stage: any) => stage.quality === 'Qualified')?.share || 0) * 100,
  ...
  },
  {
   ...
   share: Number(stages.find((stage: any) => stage.quality === 'Awaiting Document and Payment')?.share || 0) * 100,
   color: 'var(--status-warn-fill)',
  },
  {
   key: 'converted',
   quality: 'Converted',
   ...
   share: Number(stages.find((stage: any) => stage.quality === 'Converted')?.share || 0) * 100,
   color: 'var(--status-good-fill)',
  },
  {
   key: 'not_qualified',
   ...
   share: Number(stages.find((stage: any) => stage.quality === 'Not Qualified')?.share || 0) * 100,
   color: 'var(--status-bad-fill)',
  },
  {
   key: 'lost',
   quality: 'Lost',
   ...
   share: Number(stages.find((stage: any) => stage.quality === 'Lost')?.share || 0) * 100,
  ...
  <div className="page-content lead-management-page">
   ...
   <section className="dataset-section">
    <div className="lead-bento">
     <section className="lead-insights" aria-labelledby="lead-insights-heading">
      <div className="lead-insights-head">
       <div>
       ...
       <article className="lead-chart-panel lead-spend-outcomes">
        ...
        {!showSkeleton && !!summary.total && (
         <div className="lead-spend-summary" aria-label="Spend outcome totals">
          <span><b>{money(spendOutcomeSummary.spend)}</b><small>matched spend</small></span>
          <span><b>{fmt(spendOutcomeSummary.qualified)}</b><small>qualified+</small></span>
          <span><b>{fmt(spendOutcomeSummary.converted)}</b><small>converted</small></span>
          <span><b>{summary.cost_per_qualified == null ? '-' : cplMoney(summary.cost_per_qualified)}</b><small>cost / qualified</small></span>
         </div>
        )}
