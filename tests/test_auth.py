...
class AccessTests(unittest.TestCase):
    ...
    def test_valid_owner_token_is_accepted(self):
        ok, status, _, _ = verify(_request(_token()))
        self.assertTrue(ok)
        self.assertEqual(status, 200)

    def test_missing_token_is_rejected(self):
        ok, status, _, _ = verify(_request())
        self.assertFalse(ok)
    ...
    def test_token_signed_by_another_key_is_rejected(self):
        ...
        ok, status, _, _ = verify(_request(forged))
        self.assertFalse(ok)
        self.assertEqual(status, 401)
    ...
    def test_token_for_a_different_access_app_is_rejected(self):
        ...
        ok, status, _, _ = verify(_request(_token(aud="some-other-app")))
        self.assertFalse(ok)
        self.assertEqual(status, 401)

    def test_token_from_a_different_issuer_is_rejected(self):
        ok, status, _, _ = verify(_request(_token(issuer="https://attacker.cloudflareaccess.com")))
        self.assertFalse(ok)
        self.assertEqual(status, 401)

    def test_expired_token_is_rejected(self):
        ok, status, _, _ = verify(_request(_token(expires_in=-60)))
        self.assertFalse(ok)
        self.assertEqual(status, 401)
    ...
    def test_unlisted_email_is_rejected_even_with_a_valid_signature(self):
        ...
        ok, status, _, _ = verify(_request(_token(email=STRANGER)))
        self.assertFalse(ok)
        self.assertEqual(status, 403)

    def test_reader_may_read(self):
        ok, status, _, _ = verify(_request(_token(email=CEO)))
        self.assertTrue(ok)
        self.assertEqual(status, 200)

    def test_reader_may_not_delete(self):
        """The CEO gets the dashboard without the ability to erase a lead by mistake."""
        ok, status, _, _ = verify(_request(_token(email=CEO), method="DELETE", path="/api/leads/1"))
        self.assertFalse(ok)
        self.assertEqual(status, 403)

    def test_writer_may_delete(self):
        ok, status, _, _ = verify(_request(_token(email=OWNER), method="DELETE", path="/api/leads/1"))
        self.assertTrue(ok)

    def test_email_matching_is_case_insensitive(self):
        ok, _, _, _ = verify(_request(_token(email=OWNER.upper())))
        self.assertTrue(ok)

    def test_health_stays_open_for_container_checks(self):
        ok, status, _, _ = verify(_request(path="/api/health"))
        self.assertTrue(ok)
        self.assertEqual(status, 200)
    ...
    def test_cookie_is_accepted_for_browser_navigation(self):
        ...
        ok, _, _, _ = verify(request)
        self.assertTrue(ok)

    def test_empty_writer_list_lets_every_reader_write(self):
        auth.config.writers = frozenset()
        ok, _, _, _ = verify(_request(_token(email=CEO), method="DELETE", path="/api/leads/1"))
        self.assertTrue(ok)
...
class ModePrecedenceTests(unittest.TestCase):
    ...
    def test_tailscale_header_alone_does_not_authenticate(self):
        ...
        ok, status, _, _ = verify(_ts_request(OWNER))
        self.assertFalse(ok)
        self.assertEqual(status, 401)

    def test_valid_jwt_still_works(self):
        ok, _, _, _ = verify(_request(_token()))
        self.assertTrue(ok)
...
class DisabledTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        auth.config.aud = ""
        auth.config.tailscale = False
        auth.config.basic_user = ""
        auth.config.basic_pass = ""

    def test_everything_passes_when_not_configured(self):
        ok, status, _, _ = verify(_request(method="DELETE", path="/api/leads/1"))
        self.assertTrue(ok)
        self.assertEqual(status, 200)
...
def _basic_request(
    ...
    """A request carrying (or not) an RFC 7617 Authorization header."""
    import base64
...
class BasicAuthTests(unittest.TestCase):
    """The last-resort mode for a host with no Cloudflare tunnel and no tailnet -- a single
    shared credential, weaker than the other two but still fail-closed."""

    def setUp(self):
        auth.config.team_domain = ""
        auth.config.aud = ""
        auth.config.tailscale = False
        auth.config.basic_user = "demo"
        auth.config.basic_pass = "correct-horse"
    ...
    def test_correct_credentials_are_accepted(self):
        ok, status, _, _ = verify(_basic_request(("demo", "correct-horse")))
        self.assertTrue(ok)
        self.assertEqual(status, 200)

    def test_wrong_password_is_rejected(self):
        ok, status, _, _ = verify(_basic_request(("demo", "wrong")))
        self.assertFalse(ok)
        self.assertEqual(status, 401)

    def test_missing_header_fails_closed(self):
        ok, status, _, _ = verify(_basic_request(None))
        self.assertFalse(ok)
        self.assertEqual(status, 401)

    def test_challenge_header_is_sent_on_rejection(self):
        """This is what makes a browser show its native login prompt instead of a bare 401."""
        _, _, _, headers = verify(_basic_request(None))
        self.assertIn("WWW-Authenticate", headers)

    def test_health_stays_open_for_container_checks(self):
        ok, status, _, _ = verify(_basic_request(None, path="/api/health"))
        self.assertTrue(ok)
        self.assertEqual(status, 200)
